#include "traceup_config.hpp"
#include "traceup_export.h"
...
TRACEUP_EXPORT void Init_traceup()
{
  using namespace traceup::ruby;

  VALUE mTT = rb_define_module("TT");
  VALUE mPlugins = rb_define_module_under(mTT, "Plugins");
  VALUE mTraceUp = rb_define_module_under(mPlugins, "TraceUp");

  rb_define_const(mTraceUp, "CEXT_VERSION", GetVALUE(TRACEUP_VERSION));

  rb_define_module_function(mTraceUp, "trace_map", VALUEFUNC(trace_map), 2);

  rb_define_module_function(mTraceUp, "trace_polygons", VALUEFUNC(trace_polygons), 3);
}
